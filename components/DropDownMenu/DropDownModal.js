...
class DropDownModal extends PureComponent {
  ...
  getVisibleOptions() {
    ...
    return (
      visibleOptions ||
      style.visibleOptions ||
      DropDownModal.DEFAULT_VISIBLE_OPTIONS
    );
  }
  ...
  renderRow(option) {
    const { style, titleProperty, renderOption } = this.props;
  ...
  render() {
    ...
    const dataSource = this.ds.cloneWithRows(
      options.filter(option => option[titleProperty])
    );
